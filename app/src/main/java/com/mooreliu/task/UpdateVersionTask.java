package com.mooreliu.task; /*import android.app.AlertDialog.Builder;*/
...
public class UpdateVersionTask extends AsyncTask<Void, Integer, Boolean> {
    ...
    public Boolean doInBackground(Void... params) {
        ...
        if (downloadFile(urlstr, filePath, fileName) != null) return true;
        else return false;
    }
    ...
    public File downloadFile(String urlStr, String path, String fileName) {
        InputStream inputStream = mNetUtil.getInputStreamFromURL(urlStr);
        apkSize = mNetUtil.getInputStreamLength();
        File outputFile = writeFileToSDcard(path, fileName, inputStream);
        return outputFile;
    } /*     private InputStream getInputStreamFromURL(String urlStr) { URL url = null; HttpURLConnection conn = null; InputStream inputStream = null; try { url = new URL(urlStr); conn = (HttpURLConnection) url.openConnection(); conn.setReadTimeout(5 * 1000); conn.setConnectTimeout(5 * 1000); conn.setRequestMethod("GET"); conn.connect(); if(conn.getResponseCode() == HttpURLConnection.HTTP_OK) { LogUtil.e(TAG, "download responseCode = HTTP_OK"); inputStream = conn.getInputStream(); apkSize = conn.getContentLength(); LogUtil.e(TAG, "conn.getContentLength()"+conn.getContentLength()); } } catch (Exception e) { e.printStackTrace(); } return inputStream; }*/

    private File writeFileToSDcard(String filePath, String fileName, InputStream inputStream) {
        if (filePath == null || fileName == null || inputStream == null) return null;
        File file = null;
