 ...
 */
public class EventObservable implements IEventObservable { /* Singleton mode*/
	...
	public synchronized static EventObservable getInstance() {
		if (mEventObservable == null) mEventObservable = new EventObservable();
		return mEventObservable;
	}

	@Override
	public void registerObserver(EventObserver observer, String eventType) { /*LogUtil.e(TAG, " registerObserver eventType= "+eventType+" observer "+observer); LogUtil.e(TAG, " registerObserver notifyObservers mEventObservers.size() "+mEventObservers.size());*/
		if (observer != null && eventType != null) synchronized (mEventObservers) {
			ArrayList eventObservers = mEventObservers.get(eventType);
			if (eventObservers == null) {
				eventObservers = new ArrayList<EventObserver>(); /*LogUtil.e(TAG, "eventObservers == null");*/
			}
			if (eventObservers.contains(observer)) { //LogUtil.e(TAG, "eventObservers.contains(observer)  == true");
				return;
			}
